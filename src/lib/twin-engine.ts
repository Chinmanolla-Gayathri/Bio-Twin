...
export function simulate(
  ...
  const junkFoodLevel = clamp((inputs.junkFood ?? 2) / 7); // 0-1, how much junk food
  const fatigueBase =
    sleepDeficit * 0.5 + (inputs.stress / 10) * 0.35 + waterDeficit * 0.2 + (inputs.alcohol / 30) * 0.2;
  // Period stress is computed below, so we use a two-pass approach
  const periodIssueCount = (inputs.periodIssues ?? []).length;
  const isPeriodIrregular = inputs.periodCycle === "irregular";
  const isPeriodMissed = inputs.periodCycle === "missed";
  const periodStress = isPeriodMissed ? 0.20 : isPeriodIrregular ? 0.12 : 0;
  const periodFatigueBoost = isPeriodMissed ? 0.12 : isPeriodIrregular ? 0.07 : 0;
  const fatigue = clamp((fatigueBase + periodFatigueBoost) * (0.6 + 0.6 * t));

  const skinDull = clamp(
    (inputs.smoking / 30) * 0.4 + (inputs.alcohol / 30) * 0.25 + waterDeficit * 0.25 + fatigue * 0.3 + junkFoodLevel * 0.15,
  );
  ...
  // Period cycle impact on dark circles (female only)
  const periodDarkCircleBoost = isPeriodMissed ? 0.18 : isPeriodIrregular ? 0.10 : 0;
  const periodIssueDarkCircleBoost = clamp(periodIssueCount * 0.04); // each issue adds 4%

  // Dark circles: heavily driven by sleep deficit and stress
  const sleepStressBase = (sleepDeficit > 0.1 ? 0.08 : 0) + (inputs.stress > 3 ? 0.05 : 0); // minimum visible when sleep/stress is off
  const darkCircles = clamp(
    sleepDeficit * 0.40 + fatigue * 0.25 + skinDull * 0.15 + (inputs.stress / 10) * 0.25 +
    junkFoodLevel * 0.08 + periodDarkCircleBoost + periodIssueDarkCircleBoost + sleepStressBase,
  );
  ...
  // Junk food contribution to organ damage
  const junkFoodDmg = junkFoodLevel * 0.25;

  const lungsDmg = (inputs.smoking / 25) * 0.9 + (inputs.exercise < 1 ? 0.15 : 0) + condAsthma + condCOPD + (notesHasBreathing ? 0.10 : 0);
  const liverDmg = (inputs.alcohol / 20) * 0.85 + (inputs.calories > maintenance + 600 ? 0.2 : 0) + condDiabetes * 0.3 + condObesity * 0.15 + condThyroid * 0.10 + junkFoodDmg * 0.6;
  const heartDmg =
    ...
    (notesHasHeartHistory ? 0.10 : 0) + junkFoodDmg * 0.3;
  ...
  const stomachDmg =
    ...
    (inputs.alcohol / 30) * 0.2 + condAnxiety * 0.15 + condUlcer + junkFoodDmg * 0.8;

  // Period cycle impact on reproductive damage (female only)
  const periodReproDmg = isPeriodMissed ? 0.30 : isPeriodIrregular ? 0.18 : 0;
  const periodIssueDmg = clamp(periodIssueCount * 0.05);
  const reproDmg = (inputs.smoking / 30) * 0.3 + (inputs.alcohol / 30) * 0.3 + fatigue * 0.3 + condPCOS + condThyroid * 0.10 + periodReproDmg + periodIssueDmg;
  ...
  const causes: Record<OrganId, string> = {
    ...
    heart: buildHeartCause(inputs, condHypertension, condHeartDisease, notesHasHeartHistory, fatLevel, maintenance, junkFoodLevel),
    lungs: buildLungsCause(inputs, condAsthma, condCOPD, notesHasBreathing),
    liver: buildLiverCause(inputs, condDiabetes, condObesity, maintenance, junkFoodLevel),
    kidneys: buildKidneysCause(inputs, condKidneyDisease, condHypertension, waterDeficit),
    stomach: buildStomachCause(inputs, condUlcer, maintenance, junkFoodLevel),
  ...
  const risks = {
    obesity: clamp((bmi - 24) / 10 + Math.max(0, dailyDelta) / 1500 + condDiabetes * 0.15 + condObesity * 0.2 + junkFoodLevel * 0.12),
    ...
    diabetes: clamp(
      Math.max(0, fatLevel) * 0.5 + (inputs.exercise < 1 ? 0.2 : 0) +
        ...
        (notesHasDiabetesHistory ? 0.15 : 0) + junkFoodLevel * 0.15,
    ),
  };

  // ---- Disease tendency predictions ----
  const diseaseTendencies = predictDiseaseTendencies(inputs, bmi, fatLevel, fatigue, t, notesLower, risks, junkFoodLevel);
...
function buildHeartCause(
  ...
  if (junkFoodLevel > 0.5) {
    factors.push("frequent junk food consumption elevating LDL cholesterol, triglycerides, and promoting systemic inflammation via trans fats and refined sugars");
  } else if (junkFoodLevel > 0.25) {
    factors.push("regular junk food intake contributing to elevated sodium and unhealthy lipid profiles");
  }
...
function buildLiverCause(
  ...
  if (junkFoodLevel > 0.5) {
    factors.push("frequent junk food consumption flooding the liver with refined sugars and trans fats, driving de novo lipogenesis and hepatic insulin resistance");
  } else if (junkFoodLevel > 0.25) {
    factors.push("regular processed food intake contributing to hepatic fat accumulation and reduced beta-oxidation");
  }
...
function buildStomachCause(
  ...
  if (junkFoodLevel > 0.5) {
    factors.push("frequent junk food irritating gastric lining with excessive salt, preservatives, and spice, disrupting gut microbiome diversity and promoting dysbiosis");
  } else if (junkFoodLevel > 0.25) {
    factors.push("regular processed food intake impairing gut motility and reducing beneficial microbiota");
  }
...
function buildReproCause(
  ...
  if (inputs.periodCycle === "missed") {
    factors.push("missed periods (amenorrhea) indicating significant hormonal disruption — possible hypothalamic suppression, low body fat, or stress-induced HPA axis override of GnRH pulsatility");
  } else if (inputs.periodCycle === "irregular") {
    factors.push("irregular menstrual cycles suggesting disrupted HPG axis signaling, often linked to stress, weight fluctuations, or early PCOS indicators");
  }

  const periodIssues = inputs.periodIssues ?? [];
  if (periodIssues.includes("Cramps")) {
    factors.push("dysmenorrhea (severe cramps) suggesting elevated prostaglandin levels and potential endometriosis");
  }
  if (periodIssues.includes("Heavy Bleeding")) {
    factors.push("menorrhagia (heavy bleeding) indicating possible hormonal imbalance, fibroids, or coagulation irregularities");
  }
  if (periodIssues.includes("Mood Swings")) {
    factors.push("significant mood disturbances during cycle indicating marked PMS/PMDD with neurotransmitter fluctuations affecting serotonin and GABA");
  }
  if (periodIssues.includes("Acne Flare-ups")) {
    factors.push("cyclic acne indicating androgen sensitivity and sebaceous gland hyperreactivity to hormonal shifts");
  }
...
function predictDiseaseTendencies(
    ...
    if (tendency > 0.1) {
      tendencies.push({
        ...
        reason: buildTendencyReason(tendency, [
          ...
          [junkFoodLevel > 0.4, "frequent junk food (refined sugars & carbs)"],
        ]),
        category: "lifestyle",
      });
    }
  ...
  if (tendencyLiver > 0.15) {
    tendencies.push({
      ...
      reason: buildTendencyReason(tendencyLiver, [
        ...
        [junkFoodLevel > 0.4, "frequent junk food driving hepatic lipogenesis"],
      ]),
      category: "lifestyle",
    });
  }
  ...
  if (isFemale && !hasCondition("PCOS")) {
    const tendency = clamp(
      ...
      (inputs.periodCycle === "irregular" ? 0.15 : 0) +
      (inputs.periodCycle === "missed" ? 0.20 : 0),
    );
    if (tendency > 0.1) {
      tendencies.push({
        name: "PCOS",
        ...
        reason: buildTendencyReason(tendency, [
          ...
          [inputs.periodCycle === "irregular" || inputs.periodCycle === "missed", "menstrual irregularity"],
        ]),
  ...
  // --- Menstrual Disorder tendency (female only) ---
  if (isFemale && inputs.periodCycle !== "regular" && inputs.periodCycle !== "") {
    const periodIssueSevere = (inputs.periodIssues ?? []).some(i => i === "Heavy Bleeding" || i === "Missed Periods");
    const tendency = clamp(
      (inputs.periodCycle === "missed" ? 0.35 : 0.20) +
      (periodIssueSevere ? 0.15 : 0) +
      (inputs.stress > 6 ? 0.10 : 0) +
      (fatLevel > 0.3 ? 0.08 : 0) +
      ((inputs.periodIssues ?? []).length * 0.05),
    );
    if (tendency > 0.15) {
      tendencies.push({
        ...
        tendency,
        reason: buildTendencyReason(tendency, [
          [inputs.periodCycle === "missed", "missed periods (amenorrhea)"],
          [inputs.periodCycle === "irregular", "irregular cycle timing"],
          [periodIssueSevere, "severe period issues reported"],
          [inputs.stress > 6, "stress disrupting hormonal balance"],
        ]),
        category: "gender-specific",
      });
    }
