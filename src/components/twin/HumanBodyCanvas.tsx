...
/* ====== Enhanced Human Body - realistic with gender differences, muscle structure ====== */
...
function HumanBody({
  ...
  const { fatLevel, fatigue, posture, breathing, skinDull, darkCircles } = state.body;
  ...
  return (
    <group ref={group} position={[0, -0.2, 0]}>
      ...
      {/* ===== FACE ===== (smooth mannequin - no facial features) */}
      <group position={[0, 1.95, 0]}>
        {/* Smooth face plate - replaces brow, cheekbones, chin, jaw */}
        <mesh position={[0, -0.03, 0.14]} scale={[1.02, 1.05, 0.55]}>
          <sphereGeometry args={[0.21, 28, 22]} />
          {skin}
        </mesh>

        {/* Subtle nose ridge */}
        <mesh position={[0, -0.02, 0.22]} scale={[0.2, 0.3, 0.3]} rotation={[0.2, 0, 0]}>
          <capsuleGeometry args={[0.03, 0.06, 8, 10]} />
          {skin}
        </mesh>

        {/* Nose tip */}
        <mesh position={[0, -0.06, 0.25]} scale={[0.55, 0.45, 0.5]}>
          <sphereGeometry args={[0.04, 10, 10]} />
          {skin}
        </mesh>

        {/* Eye socket depressions - subtle concave areas */}
        {[-1, 1].map((s) => (
          <mesh key={`eye-socket-${s}`} position={[s * 0.09, 0.0, 0.17]} scale={[0.5, 0.3, 0.15]}>
            ...
            <meshStandardMaterial
              ...
              metalness={0.02}
              transparent={transparent}
              opacity={transparent ? 0.08 : 0.35}
              depthWrite={!transparent}
            />
          </mesh>
        ))}

        {/* Dark circles under eyes - highly visible when sleep deficit or high stress */}
        {!transparent && darkCircles > 0.01 && [-1, 1].map((s) => (
          <group key={`dark-circle-${s}`}>
            {/* Main dark circle patch - large and prominent */}
            <mesh position={[s * 0.09, -0.045, 0.19]} scale={[0.85, Math.max(0.4, darkCircles * 1.3), 0.3]}>
              <sphereGeometry args={[0.06, 14, 12]} />
              <meshStandardMaterial
                color={new THREE.Color("#2a1a35").lerp(new THREE.Color("#0d0518"), Math.min(1, darkCircles * 1.5))}
                roughness={0.65}
                metalness={0.0}
                transparent
                opacity={Math.min(0.95, 0.3 + darkCircles * 0.65)}
                depthWrite={false}
              />
            </mesh>
            {/* Inner shadow near nose - deep and dark */}
            <mesh position={[s * 0.055, -0.04, 0.20]} scale={[0.55, Math.max(0.35, darkCircles * 1.1), 0.25]}>
              <sphereGeometry args={[0.045, 12, 10]} />
              <meshStandardMaterial
                color={new THREE.Color("#1f0f2e").lerp(new THREE.Color("#080312"), Math.min(1, darkCircles * 1.5))}
                roughness={0.7}
                metalness={0.0}
                transparent
                opacity={Math.min(0.9, 0.25 + darkCircles * 0.65)}
                depthWrite={false}
              />
            </mesh>
            {/* Outer shadow spreading outward */}
            <mesh position={[s * 0.14, -0.035, 0.17]} scale={[0.55, Math.max(0.3, darkCircles * 1.0), 0.22]}>
              <sphereGeometry args={[0.04, 10, 10]} />
              <meshStandardMaterial
                color={new THREE.Color("#3a2548").lerp(new THREE.Color("#120820"), Math.min(1, darkCircles * 1.3))}
                roughness={0.65}
                metalness={0.0}
                transparent
                opacity={Math.min(0.8, 0.2 + darkCircles * 0.6)}
                depthWrite={false}
              />
            </mesh>
            {/* Secondary ring below eye - makes it look like real dark circles */}
            <mesh position={[s * 0.09, -0.06, 0.17]} scale={[0.65, Math.max(0.25, darkCircles * 0.8), 0.2]}>
              <sphereGeometry args={[0.04, 10, 10]} />
              <meshStandardMaterial
                color={new THREE.Color("#352040").lerp(new THREE.Color("#0a0415"), Math.min(1, darkCircles * 1.2))}
                roughness={0.7}
                metalness={0.0}
                transparent
                opacity={Math.min(0.7, 0.15 + darkCircles * 0.55)}
                depthWrite={false}
              />
            </mesh>
            {/* Lower cheek discoloration for severe dark circles */}
            {darkCircles > 0.3 && (
              <mesh position={[s * 0.08, -0.085, 0.15]} scale={[0.6, Math.max(0.3, darkCircles * 0.9), 0.18]}>
                <sphereGeometry args={[0.04, 10, 10]} />
                <meshStandardMaterial
                  color={new THREE.Color("#2a1535").lerp(new THREE.Color("#06020e"), darkCircles)}
                  roughness={0.75}
                  metalness={0.0}
                  transparent
                  opacity={Math.min(0.65, 0.1 + darkCircles * 0.55)}
                  depthWrite={false}
                />
              </mesh>
            )}
      ...
      {/* Ears - simplified smooth for mannequin look */}
      {[-1, 1].map((s) => (
        <group key={`ear-${s}`} position={[s * 0.23, 1.93, -0.02]} rotation={[0, s * -0.2, s * 0.1]}>
          {/* Simple smooth ear */}
          <mesh scale={[0.25, 0.5, 0.18]}>
